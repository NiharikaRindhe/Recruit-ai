...
def _to_bullets(s: str, max_items: int = 6, max_len: int = 180) -> str:
    ...
    # If regex didn't find bullets, split by sentence boundaries roughly
    if len(parts) <= 1:
    ...
    for p in parts:
        ...
        if not p:
            continue
        # CHANGED: Removed the truncation logic (p[:max_len]...) to prevent incomplete sentences
        clean.append(p)
...
def _recent_turns(session_id: str, limit: int = 2000) -> list[dict]:
    ...
    # CHANGED: If limit is 0, return the full transcript (needed for validation search)
...
async def ai_expected(payload: Dict[str, str] = Body(...)):
    ...
    # Only need recent context to understand if there are specific constraints
    turns = _recent_turns(session_id, limit=2000) 
    convo = _turns_snippet(turns, 12)

    prompt = f"""
You are generating an Expected Answer for the interviewer. 
The interviewer asked: "{question}"

Return STRICT JSON with exactly one key: "expected_answer".

Instructions:
1. Analyze the JD to see what requirements this question relates to.
2. Analyze the RESUME to see if the candidate has relevant skills/projects.
3. Construct a point-wise answer that bridges the JD requirements with the Candidate's Resume.
   - e.g. "According to the JD, knowledge of X is needed... Based on the Resume, the candidate can mention project Y..."
4. CRITICAL: Write complete, grammatically correct sentences. Do not use "..." or trail off. 
5. Provide 3-4 concise bullet points.
...
""".strip()
    ...
    return {
        ...
        "question": question,
        "expected_answer": _to_bullets(exp_raw, max_items=4),
    }
...
async def ai_validate(payload: Dict[str, str] = Body(...)):
    ...
    # CHANGED: Fetch ALL turns (limit=0) to search for the question
    turns = _recent_turns(session_id, limit=0) 
    
    # Logic: Find the index of the question in the transcript
    target_idx = -1
    q_norm = _norm(question)
    
    # Search backwards to find the most recent occurrence
    for i in range(len(turns) - 1, -1, -1):
        t_text = _norm(turns[i].get("text", ""))
        # Fuzzy match: if a significant part of the question is in the turn
        if q_norm in t_text or t_text in q_norm or (len(q_norm) > 10 and q_norm[:20] in t_text):
            target_idx = i
            break
            
    context_turns = []
    if target_idx != -1:
        # Slice: 7 turns BEFORE and 7 turns AFTER the question
        start_i = max(0, target_idx - 7)
        end_i = min(len(turns), target_idx + 8) 
        context_turns = turns[start_i:end_i]
    else:
        # Fallback if exact question not found: take last 15 turns
        context_turns = turns[-15:]

    # Format transcript window for LLM
    lines = [
        f"{(t.get('speaker_name') or t.get('speaker') or t.get('source'))}: {t.get('text', '')}"
    ...
    prompt = f"""
You are validating a candidate's answer. 
Return STRICT JSON with keys: "verdict", "explanation".

INPUTS:
- Question asked: "{question}"
- Context: Transcript window (7 turns before/after question).

VERDICT RULES:
- "STRONG": The candidate answered the question properly and completely.
- "OK": The candidate gave an incomplete answer or only half-matched the requirement.
- "LIMITED": The candidate did not answer, gave a wrong answer, or the answer is irrelevant.

MATCH THIS SCORE WITH VERDICT:
...
EXPLANATION RULES:
- Write 2-3 complete sentences explaining the verdict. 
...
""".strip()
    ...
    # Normalize verdict to your requested set
    if "ALMOST" in verdict or "STRONG" in verdict: 
        verdict = "ALMOST"
    elif "OK" in verdict: 
        verdict = "OK"
    else: 
    ...
    return {
        ...
        "question": question,
        "verdict": verdict,
        "explanation": _to_bullets(explanation_raw, max_items=3),
        # We return the window used so you can debug if needed
        "transcript_window_used": transcript_window
    }
...
async def ai_questions(payload: Dict[str, Any] = Body(...)):
    ...
    # Get enough context for "flow"
    turns = _recent_turns(session_id, limit=2000)
    convo = _turns_snippet(turns, 10)

    prompt = f"""
You are suggesting follow-up interview questions.
Return STRICT JSON with key: "items" (array of objects with "question" and "answer").

RULES:
1. Questions must follow the FLOW of the current conversation (Context).
2. Questions must relate to the JD and Resume.
3. "answer": Provide a suggested answer for the candidate.
   - This must be point-wise.
   - CRITICAL: Use COMPLETE sentences. Do not leave sentences incomplete.
...
Return JSON with exactly 3 items: {{ "items": [{{"question":"...","answer":"..."}}] }}
""".strip()

    data = _ollama_chat_json(prompt, temperature=0.4)
    ...
    if isinstance(items, list):
        for item in items:
            ...
            if q and a_raw:
                ...
                answers.append(_to_bullets(a_raw, max_items=3))

    # Fallback if empty or malformed
    if not questions:
        questions = ["Could you elaborate on your recent project?", "How does your experience align with this role?", "What is your preferred tech stack?"]
        answers = ["Candidate should describe project scope and impact.", "Candidate should map skills to JD requirements.", "Candidate should explain their tool choices."]
